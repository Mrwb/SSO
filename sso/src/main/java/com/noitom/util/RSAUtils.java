...
//		preLogin();
    	try {
//			encryptByPublicKey("abac".getBytes(),getPublicKey());
			System.out.println(new String(decryptByPrivateKey(encryptByPublicKey("abacabac".getBytes())),"UTF-8"));
//			String sign = sign("abc".getBytes(),  getPrivateKey());
//			System.out.println(verify("abc".getBytes(), getPublicKey(), sign));
    	} catch (Exception e) {
			e.printStackTrace();
		}
//		System.out.println(decrypt(encrypt("abc")));
	}
    ...
    static void testSign() throws Exception {
        ...
        byte[] decodedData = RSAUtils.decryptByPublicKey(encodedData, getPublicKey());
        String target = new String(decodedData);
        System.out.println("解密后: \r\n" + target);
        System.err.println("私钥签名——公钥验证签名");
        String sign = RSAUtils.sign(encodedData, getPrivateKey());
        System.err.println("签名:\r" + sign);
        boolean status = RSAUtils.verify(encodedData, getPublicKey(), sign);
